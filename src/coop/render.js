...
import {ACTIVE,PASSIVES,skillName} from './builds.js';
...
export class View{
 ...
 draw(dt=0){
  ...
  if(w.options.feedback){for(const f of w.effects)if(f.type!=='number')effect(c,f);for(const h of w.heroes)if(actionLayout(h)?.layer==='ground')heroAction(c,h);}
  ...
  const actors=[...SCENE_PROPS.map(p=>({...p,x:p.x*MAP_SCALE,y:p.y*MAP_SCALE,prop:true})),...w.obstacles.map(o=>({...o,obstacle:true})),...w.enemies,...w.heroes,...w.projectiles.map(p=>({...p,projectile:true})),...(w.options.feedback?w.heroes.filter(h=>actionLayout(h)?.layer==='depth').map(h=>({...actionLayout(h),actionHero:h})):[])].sort((a,b)=>a.y-b.y);
 ...
 hud(){const c=this.c,w=this.world;
  ...
  if(w.mode==='play'&&!w.enemies.length)text(c,'清波奖励即将开启…',720,153,23,CREAM,'center');
 ...
 playerHUD(i,x){const c=this.c,w=this.world,h=w.heroes[i],color=colors[i];
  ...
  text(c,Object.entries(h.passives).map(([key,rank])=>`${PASSIVES[key].title}${rank}`).join(' · ')||'被动：0 / 4',x+101,757,10,'#cdd7b1');
 ...
 upgrade(){const c=this.c,w=this.world,skills=w.rewardType==='skill';this.veil(.82);text(c,skills?'清波 · 选择构筑':'升级 · 强化属性',720,70,36,CREAM,'center');text(c,skills?'2 个主动槽 · 4 个被动槽 · 主动 III + 对应被动 II 可进化':`小队 Lv.${w.level} · 选完立即回到战斗，经验溢出保留`,720,111,17,'#c9d7b8','center');
  for(let i=0;i<w.humanCount;i++){const x=w.humanCount===1?450:126+i*648,col=colors[i],h=w.heroes[i];text(c,`P${i+1} · ${ROLES[h.role].name}`,x+270,157,22,col,'center');
   ...
   ACTIVE[h.role].forEach((a,slot)=>{text(c,`${slot?'E':'Q'}  ${h.skills[slot]?skillName(h,slot)+' '+h.skills[slot]:'未习得 '+a.title}  →  ${a.title} III + ${PASSIVES[a.need].title} II`,x+10,632+slot*23,13,'#e7d9ad');});
