...
export function startBossSkill(w,e,key){
 ...
 e.action={kind:key,type:'boss',t:0,windup:key==='ultimate'?2.1:key==='summon'?1.35:key==='barrage'?.95:1.1,x:target.x,y:target.y,r:key==='slam'?155:0,hit:false};
 const a=e.action,add=(x,y,r,delay,amount)=>w.bossWarnings.push({owner:e.id,skill:key,x,y,r,t:0,windup:delay,damage:amount,hit:false});
