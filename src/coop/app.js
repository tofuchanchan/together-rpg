...
const actions={start(roles){unlockAudio();world.reset(roles,view?.humanCount||2);clearTransition();document.querySelector('#status').textContent=world.humanCount===1?'单人冒险 · 双 AI 队友':'双人战斗 · 本地同屏';},pause(){if(world.mode==='play'||world.mode==='upgrade')world.pause();else if(world.mode==='paused'&&!router.slots.slice(0,world.humanCount).some(s=>s.type==='disconnected'))world.resume();clearTransition();},confirm(slot){world.confirm(slot);clearTransition();},proceed(){if(world.mode==='complete')world.nextRoom();else world.reset(view.roles,view.humanCount);clearTransition();},menu(){world.mode='menu';clearTransition();}};
...
function update(seconds){const pads=testPads??Array.from(navigator.getGamepads?.()||[]),input=router.sample(pads);const missing=router.disconnected.filter(i=>i<world.humanCount);if(missing.length)world.pause(`P${missing[0]+1} 设备断开，请重新绑定`);
 ...
 if(world.mode==='upgrade'){let confirmed=false;for(let i=0;i<world.humanCount;i++){if(input[i].up)world.choose(i,(world.selection[i]+2)%3);if(input[i].down)world.choose(i,(world.selection[i]+1)%3);if(input[i].confirm&&!world.ready[i]){world.confirm(i);confirmed=true;}}if(confirmed)router.flush();return;}
...
window.coopTest={world,router,get view(){return view;},start:(roles=['warrior','mage'],humanCount=2)=>{manual=true;view.roles=[...roles];view.humanCount=humanCount;actions.start(roles);view.draw();},setPads:p=>testPads=p,resumeRealtime:()=>{manual=false;testPads=null;},render:()=>view.draw(),game};
