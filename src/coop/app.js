...
loadHeroPortraits();
try{await Promise.all([loadCharacterSprites(),loadWorldArt(),loadBuildArt(),loadUniversalArt(),loadEquipmentArt(),loadShopEventArt()]);document.querySelector('#status').textContent='林间远征 · 通用构筑 / 连续增援 / 拾取成长';}
...
window.render_game_to_text=()=>JSON.stringify({assetsReady:!!window.assetsReady,frontend:startScreen.snapshot(),renderSurface:renderSurface?.snapshot(),heroPortraits:heroPortraitState(),characterArt:characterAssetState(),worldArt:worldArtState(),buildArt:buildArtState(),universalArt:universalArtState(),equipmentArt:equipmentAssetState(),shopEventArt:shopEventArtState(),...world.snapshot(),codex:codex.snapshot(),rewardChoices:world.heroes.map((_,i)=>world.rewardChoices(i)),devices:router.slots,awaitingPad:router.awaiting,uiNavigation:view.navigationSnapshot()});
