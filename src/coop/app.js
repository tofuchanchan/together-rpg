import {createSceneLoading} from './scene-loading.js';
...
const sceneLoading=createSceneLoading({equipment:loadEquipmentArt,shop:()=>loadShopEventArt('icons'),bonus:()=>loadShopEventArt('bonus'),objectives:()=>loadAdventureArt('objectives'),boss:()=>loadAdventureArt('boss')},{flush:()=>{router.blur();world.clearBuffers();},onBack:()=>startScreen.showTitle()});
...
addEventListener('keydown',e=>{if(sceneLoading.key(e))return;if(startScreen.blocksGame||codex.isOpen)return;if(router.awaiting!==null){e.preventDefault();if(e.code==='Escape'){router.awaiting=null;router.flush();}return;}if(e.code==='KeyI'&&!e.repeat){e.preventDefault();codex.open();return;}if(known.has(e.code))e.preventDefault();if(!view)return;unlockAudio();router.key(e.code,true,e.repeat);if(e.repeat)return;if(e.code==='KeyF')fullscreen();if(world.mode==='menu'&&e.code==='Enter')actions.start(view.roles);else if(['complete','defeat','victory'].includes(world.mode)&&e.code==='Enter')actions.proceed();});
...
function update(seconds){
 ...
 if(!sceneLoading.check(world,{active:!startScreen.blocksGame})){sceneLoading.pad([...(router.padInputs?.values()||[])]);return;}
...
loadHeroPortraits();
try{await Promise.all([loadCharacterSprites(),loadWorldArt(),loadBuildArt(),loadUniversalArt()]);document.querySelector('#status').textContent='林间远征 · 通用构筑 / 连续增援 / 拾取成长';}
