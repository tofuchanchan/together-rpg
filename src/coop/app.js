...
addEventListener('keydown',e=>{if(startScreen.blocksGame||codex.isOpen)return;if(e.code==='KeyI'&&!e.repeat){e.preventDefault();codex.open();return;}if(known.has(e.code))e.preventDefault();if(!view)return;unlockAudio();router.key(e.code,true,e.repeat);if(e.repeat)return;if(e.code==='KeyF')fullscreen();if(world.mode==='menu'&&e.code==='Enter')actions.start(view.roles);else if(['complete','defeat','victory'].includes(world.mode)&&e.code==='Enter')actions.proceed();});
...
function update(seconds){const pads=testPads??Array.from(navigator.getGamepads?.()||[]),input=router.sample(pads);if(startScreen.blocksGame)return;const missing=router.disconnected.filter(i=>i<world.humanCount);if(missing.length)world.pause(`P${missing[0]+1} 设备断开，请重新绑定`);
