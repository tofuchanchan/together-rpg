...
export class InputRouter{
 ...
 sample(pads=[]){const byId=new Map(pads.filter(Boolean).map(p=>[p.index,p]));const prev=this.padPrevious,down=new Map();
  ...
  const result=this.slots.map(s=>{let o={x:0,y:0,skill1:false,skill2:false,dodge:false,confirm:false,reroll:false,up:false,down:false,left:false,right:false,pause:false};if(s.type==='keyboard'){const k=KEYS[s.id],held=c=>this.held.has(c)&&!this.blocked.has(c),edge=c=>!this.blocked.has(c)&&this.edges.has(c);o.x=+held(k.right)-+held(k.left);o.y=+held(k.down)-+held(k.up);const m=Math.hypot(o.x,o.y);if(m>1){o.x/=m;o.y/=m;}for(const a of ['skill1','skill2','dodge','confirm','reroll','up','down','left','right'])o[a]=edge(k[a]);o.pause=edge('Escape')||edge('KeyP');}else if(s.type==='gamepad'){const p=byId.get(s.id),b=down.get(s.id)||[],v=analog(p.axes[0]||0,p.axes[1]||0);o.x=v.x;o.y=v.y;const edge=i=>!suppress&&!!b[i];o.skill1=edge(2);o.skill2=edge(3);o.dodge=edge(0);o.confirm=edge(0);o.reroll=edge(3);o.pause=edge(9);o.up=edge(12);o.down=edge(13);o.left=edge(14);o.right=edge(15);const oldX=prev.get(s.id)?.stickX||0,stickX=(p.axes[0]||0)>.55?1:(p.axes[0]||0)<-.55?-1:0;o.left||=!suppress&&stickX===-1&&oldX!==-1;o.right||=!suppress&&stickX===1&&oldX!==1;const old=prev.get(s.id)?.stick||0,stick=(p.axes[1]||0)>.55?1:(p.axes[1]||0)<-.55?-1:0;o.up||=!suppress&&stick===-1&&old!==-1;o.down||=!suppress&&stick===1&&old!==1;}
   return o;});
