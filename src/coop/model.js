...
export class World{
 ...
 step(dt,inputs){if(this.mode!=='play')return;this.time+=dt;this.tick++;if(this.bonusEvent)tickBonusEvent(this,dt);else this.updateSpawns(dt);this.shake=Math.max(0,this.shake-dt);this.effects=this.effects.filter(f=>(f.life-=dt)>0);
  ...
  else if(!this.enemies.length&&this.waveElapsed>=this.waveDuration){this.waveTimer+=dt;if(this.waveTimer>1.1){if(this.bossRoom&&this.room>=20&&!this.endless){this.clears++;this.settleWave();this.mode='victory';this.clearBuffers();this.emit('victory');enterShop(this,'victory');}else this.beginReward('skill');return;}}
 ...
 resolveBodies(){
  ...
  for(const a of this.enemies){const gx=Math.floor(a.x/cell),gy=Math.floor(a.y/cell),ra=enemyDef(a).bodyRadius||22;
   ...
   if(a.spawnGrace>0||a.hp<=0||a.bonusKind)continue;
