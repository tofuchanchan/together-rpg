...
export class World{
 ...
 updatePickups(dt=1/120){
  ...
  for(const p of [...this.pickups]){
   ...
   const potion=p.type==='potion';let candidates=this.heroes.filter(h=>!h.down&&(p.owner===undefined||p.owner===h.id||p.type==='supply')&&(!potion||h.hp<h.maxHp)&&dist(h,p)<(potion?38:(h.pickupRadius||105))&&this.lineClear(h,p));
