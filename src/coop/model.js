...
export const MAP_SCALE=Math.sqrt(3),MAP={x:550*MAP_SCALE,y:345*MAP_SCALE};
...
export class World{
 ...
 spawnWave(){this.waveTimer=0;this.waveElapsed=0;this.bossRoom=this.room%10===0;this.bossWarnings=[];
  const plan=wavePlan(this.room,this.wave);this.waveDuration=this.bossRoom?0:plan.duration;this.batchTotal=this.bossRoom?1:plan.batches.length;this.batchSpawned=0;this.spawnQueue=[];
 ...
 updateSpawns(dt){this.waveElapsed+=dt;while(this.spawnQueue.length&&this.waveElapsed>=this.spawnQueue[0].at){if(this.enemies.length>100)break;this.spawnBatch(this.spawnQueue.shift());}}
 ...
 consume(h){const b=h.buffer;if(!b)return false;if(this.time>b.expires){h.buffer=null;return false;}const a=h.action;if(b.type==='dodge'){
  ...
  h.visualStop=0;h.hitPose=null;h.action={type,slot:s,t:0,...actionTiming(h.role,type),dir:type==='bash'?b.dir:d,hit:new Set(),fired:false};h.cd[s]=ROLES[h.role].cd[s]*(1-h.cooldown)*(h.runes[s]==='quick'?.75:h.runes[s]==='force'?1.2:h.runes[s]==='wide'?1.15:1)*(h.core==='arcanist'||h.core==='executioner'?1.15:h.core==='pyromancer'&&s===1?1.3:1);h.shield=Math.min(60,h.shield+(h.passives.guard||0)*8+(h.core==='bulwark'?25:0));h.charged=h.passives.arcane||0;h.casts++;if(h.core==='arcanist'&&h.casts%3===0)this.delayed.push({type:'arcane',owner:h.id,delay:h.action.windup,damage:this.skillDamage(h,s,40)*.6});h.buffer=null;h.face=dir8(h.action.dir.x,h.action.dir.y);this.emit('skill',{id:h.id,skill:type});return true;
 }
 ...
 step(dt,inputs){if(this.mode!=='play')return;this.time+=dt;this.tick++;this.updateSpawns(dt);this.shake=Math.max(0,this.shake-dt);this.effects=this.effects.filter(f=>(f.life-=dt)>0);
  ...
  const ps=this.heroes.slice(0,this.humanCount),cx=ps.reduce((n,h)=>n+h.x,0)/ps.length,cy=ps.reduce((n,h)=>n+h.y,0)/ps.length;const spread=this.humanCount===2?Math.max(Math.abs(ps[0].x-ps[1].x)/1800,Math.abs(ps[0].y-ps[1].y)/1500):0;const z=clamp(1.04-spread,.78,1.02)*.8;const k=1-Math.exp(-dt*3);this.camera.x+=(cx*.85-this.camera.x)*k;this.camera.y+=(cy*.8-this.camera.y)*k;if(Math.abs(z-this.camera.zoom)>.025)this.camera.zoom+=(z-this.camera.zoom)*k;
 }
 ...
 updateEnemy(e,dt){
  const d=enemyDef(e);if(e.spawnGrace>0){e.spawnGrace=Math.max(0,e.spawnGrace-dt);return;}if(e.affixes?.includes('regen')&&this.time-(e.lastHit??-9)>1.5)e.hp=Math.min(e.maxHp,e.hp+e.maxHp*.003*e.rarity*dt);e.hitFlash=Math.max(0,e.hitFlash-dt);e.visualStop=Math.max(0,e.visualStop-dt);if(e.hitReaction)e.hitReaction.life=Math.max(0,e.hitReaction.life-dt);e.slow=Math.max(0,e.slow-dt);e.freeze=Math.max(0,(e.freeze||0)-dt);
