...
import {partyPressure,pressurePlan,createPressureState,pressureTick,pressureKind} from './pressure.js';
...
export class World{
 ...
 reset(roles=['warrior','mage'],humanCount=2){this.humanCount=humanCount===1?1:2;if(roles.length<this.humanCount||roles.slice(0,this.humanCount).some(r=>!ROLES[r]))throw Error('Invalid player roles');this.shop=null;this.shopVisitedRoom=0;this.equipmentObjects=[];this.time=0;this.tick=0;this.accumulator=0;this.pendingEdges=[{},{}];this.mode='play';this.reason='';this.room=1;this.wave=1;this.level=1;this.kills=0;this.xp=0;this.xpNext=xpRequired(1);this.gold=0;this.endless=false;this.challengeUsed=false;this.challengeNext=false;this.eliteChallenge=false;this.rewardMenus=[null,null,null];this.rewardError=null;this.highReward=false;this.pets=[];this.universalObjects=[];this.rewardType=null;this.enemies=[];this.projectiles=[];this.effects=[];this.hazards=[];this.pickups=[];this.delayed=[];this.events=[];this.nextId=10;this.nextAttackId=1;this.clears=0;this.deathQueue=[];this.waveTimer=0;this.waveElapsed=0;this.spawnQueue=[];this.bossWarnings=[];this.obstacles=OBSTACLES;this.ready=[false,false];this.selection=[0,0];this.offers=[];this.camera={x:0,y:0,zoom:.8};this.shake=0;this.options={feedback:true,shake:true,sound:true};
 ...
 spawnWave(){
  ...
  this.pressure=null;this.pressureState=null;this.waveSpawned=0;
  if(this.bossRoom){this.waveDuration=0;this.batchTotal=1;this.enrageAt=rageDeadline(0,true);const boss=this.createEnemy('thornking',0,-80);boss.hp=boss.maxHp=Math.round(3700*(1+(this.room/10-1)*1.05));this.enemies.push(boss);this.batchSpawned=1;return;}
 ...
 spawnBatch(batch){
  ...
  for(let i=0;i<batch.count;i++){
   ...
   if(kind==='shaman'&&this.enemies.filter(e=>e.hp>0&&e.kind==='shaman').length>=this.heroes.length)kind='skeleton';
 ...
 pause(reason='暂停'){if(['play','upgrade','shop'].includes(this.mode)){this.resumeMode=this.mode;this.mode='paused';this.reason=reason;this.clearBuffers();}}
 ...
 consume(h){const b=h.buffer;if(!b)return false;if(this.time>b.expires){h.buffer=null;return false;}const a=h.action;if(b.type==='dodge'){
  ...
  h.visualStop=0;h.hitPose=null;h.action={type,slot:s,t:0,...actionTiming(h.role,type),facing:dir8((type==='bash'?b.dir:d).x,(type==='bash'?b.dir:d).y),dir:type==='bash'?b.dir:d,hit:new Set(),fired:false,eventId:this.nextAttackId++};configureBuildAction(this,h,h.action);
  // Basic Q locks a world point at startup; moving the caster must not translate the aim line.
  if(target&&!h.action.form&&!h.action.buildBranch&&['fireball','pierce'].includes(type))h.action.aimPoint={x:target.x,y:target.y};
 ...
 nearest(h,range=Infinity){
  let best=null,d=range,support=null,supportDistance=range,alive=0;
  for(const e of this.enemies){if(e.hp<=0)continue;alive++;const n=dist(h,e);if(n>=range||!this.lineClear(h,e))continue;if(n<d){best=e;d=n;}if(e.stats?.behavior==='healer'&&n<supportDistance){support=e;supportDistance=n;}}
  ...
  return support&&alive<=8&&d>100?support:best;
 }
 ...
 damageEnemy(e,n,h,knock=70,source='attack',extraCrit=0,incoming=null,context={}){
  if(e.hp<=0||!h)return;const ctx={...context,eventId:context.eventId??((source==='attack'||source==='skill')?h.action?.eventId:null)??this.nextAttackId++,depth:context.depth||0};if(ctx.depth>2)return;
  ...
  const beforeArmor=n;n*=1-(e.stats?.armor||0);const dealt=Math.min(e.hp,Math.max(0,n));e.lastHit=this.time;e.hp=Math.max(0,e.hp-n);h.damageDone+=dealt;
 ...
 step(dt,inputs){if(this.mode!=='play')return;this.time+=dt;this.tick++;this.updateSpawns(dt);this.shake=Math.max(0,this.shake-dt);this.effects=this.effects.filter(f=>(f.life-=dt)>0);
  for(const h of this.heroes){let input={...baseInput(),...(inputs[h.id]||{})};if(h.ai)input=this.aiInput(h);h.cd=h.cd.map(t=>Math.max(0,t-dt));h.dodgeCd=Math.max(0,h.dodgeCd-dt);h.attackCd=Math.max(0,h.attackCd-dt);h.invuln=Math.max(0,h.invuln-dt);h.hitFlash=Math.max(0,h.hitFlash-dt);h.visualStop=Math.max(0,h.visualStop-dt);if(h.hitReaction)h.hitReaction.life=Math.max(0,h.hitReaction.life-dt);
   ...
   h.empowered=Math.max(0,h.empowered-dt);tickBuild(this,h,dt);const a=h.action;const speed=ROLES[h.role].speed*h.speedBonus*buildMoveFactor(h)*(a?.form==='aegis'?0:1);
   if(a&&(a.type==='dodge'||(a.type==='bash'&&a.form!=='aegis'))){const old=a.t;a.t=Math.min(a.duration,a.t+dt);const windup=a.type==='bash'?(a.windup??0):0;const ease=t=>{const q=clamp((t*a.duration-windup)/(a.duration-windup),0,1);return 1-(1-q)**2;};const travel=a.type==='dodge'?112:145*(h.evolved[0]?1.3:1);const d=(ease(a.t/a.duration)-ease(old/a.duration))*travel;this.moveActor(h,a.dir.x*d,a.dir.y*d);h.face=dir8(a.dir.x,a.dir.y);}
   ...
   if(a){const t=a.t;if(a.slot!==undefined&&!a.buildReleased&&t>=(a.windup||0)){a.buildReleased=true;releaseBuildEnergy(this,h,a);}
    ...
    const handled=updateBuildAction(this,h,a);
    if(!handled&&a.type==='bash'&&t>=(a.windup??.045)&&t<=(a.activeEnd??a.duration)){for(const e of this.enemies)if(!a.hit.has(e.id)&&dist(h,e)<80*this.skillRange(h,0)&&this.lineClear(h,e)){a.hit.add(e.id);this.damageEnemy(e,this.skillDamage(h,0,36),h,260,'skill');if(h.evolved[0]&&!h.evolutionBranches?.[0])refundCooldown(h,0,.4);}}
  ...
  this.updateProjectiles(dt);this.updateHazards(dt);updateBossWarnings(this,dt);tickUniversal(this,dt);tickEquipment(this,dt);this.updatePickups(dt);
  ...
  if(!this.enemies.length&&this.waveElapsed>=this.waveDuration){this.waveTimer+=dt;if(this.waveTimer>1.1){if(this.bossRoom&&this.room>=20&&!this.endless){this.clears++;this.settleWave();this.mode='victory';this.clearBuffers();this.emit('victory');enterShop(this,'victory');}else this.beginReward('skill');return;}}
 ...
 updateProjectiles(dt){
  for(const p of this.projectiles){const travel=p.speed*Math.min(dt,Math.max(0,p.life))+(p.muzzle||0);p.muzzle=0;p.life-=dt;const start={x:p.x,y:p.y},end={x:p.x+p.dx*travel,y:p.y+p.dy*travel};
   ...
   for(const e of p.hostile?this.heroes:this.enemies)if(e.hp>0&&!p.hit.has(e.id))contacts.push({t:segmentCircle(start,end,e,p.hostile?17:enemyDef(e).bodyRadius||23),target:e});
   ...
   for(const hit of contacts){if(hit.t===Infinity)break;const pos={x:start.x+(end.x-start.x)*hit.t,y:start.y+(end.y-start.y)*hit.t},dir={x:p.dx,y:p.dy};
    ...
    else{const h=this.heroes[p.owner];const dealt=buildProjectileHit(this,p,e,h,dir);this.damageEnemy(e,dealt,h,p.type==='pierce'?150:65,p.source,p.evolved&&p.type==='pierce'?.25:0,dir,{eventId:p.eventId,skipBurn:p.skipBurn});
 ...
 updatePickups(dt=1/120){
  ...
  for(const p of [...this.pickups]){
   ...
   const potion=p.type==='potion';let candidates=this.heroes.filter(h=>!h.down&&(p.owner===undefined||p.owner===h.id||p.type==='supply')&&(!potion||h.hp<h.maxHp)&&dist(h,p)<(potion?38:(h.pickupRadius||75))&&this.lineClear(h,p));
   candidates.sort((a,b)=>potion||p.type==='supply'?(a.hp+(a.shield||0))/a.maxHp-(b.hp+(b.shield||0))/b.maxHp:dist(a,p)-dist(b,p));
   let h=this.heroes.find(h=>h.id===p.attractedTo&&!h.down&&(p.owner===undefined||p.owner===h.id||p.type==='supply'))||candidates[0];
   if(!h)continue;
   ...
   else if(p.type==='gold'){this.gold+=p.value||1;this.emit('gold',{id:h.id,value:p.value||1});}
 ...
 clearOwnedBuild(h){resetUniversal(this,h.id);resetEquipment(this,h.id);this.projectiles=this.projectiles.filter(p=>p.owner!==h.id);this.hazards=this.hazards.filter(p=>p.owner!==h.id);this.delayed=this.delayed.filter(p=>p.owner!==h.id);h.action=null;h.shadow=null;h.guardUntil=0;h.resource=0;h.storedGuard=0;h.counterReady=false;h.weakpointUntil=0;h.shadowRefundUntil=0;h.huntTarget=null;h.huntStacks=0;h.resourceEvents=new Set();for(const e of this.enemies){if(e.huntMarks)delete e.huntMarks[h.id];}}
 reroll(slot){
 ...
 cancelReplacement(slot){const menu=this.rewardMenus[slot];if(this.mode!=='upgrade'||this.ready[slot]||menu?.type!=='replace')return false;this.offers[slot]=menu.original;this.selection[slot]=menu.selection;this.rewardMenus[slot]=null;this.rewardError=null;return true;}
 ...
 confirm(slot){
  ...
  const key=menu?.type==='replace'?menu.pendingKey:o.key,result=this.applyHeroReward(h,key,menu?.type==='replace'?{replaceKey:o.key.replace('replace:','')}:{});
  ...
  if(menu?.type==='replace'||result?.clearOwnedObjects)this.clearOwnedBuild(h);if(menu?.type==='replace'){this.offers[slot]=menu.original;this.selection[slot]=menu.original.findIndex(o=>o.key===key);}this.rewardMenus[slot]=null;this.rewardError=null;this.ready[slot]=true;if(!this.ready.every(Boolean))return;this.finishReward();
 }
 settleWave(){
  this.projectiles=[];this.hazards=[];this.delayed=[];this.effects=[];this.bossWarnings=[];resetUniversal(this);resetEquipment(this);
  for(const hero of this.heroes){hero.action=null;hero.shadow=null;hero.guardUntil=0;hero.resource=0;hero.storedGuard=0;hero.counterReady=false;hero.charged=0;hero.empowered=0;hero.revive=0;if(hero.down){hero.down=false;hero.hp=hero.maxHp*.35;}else hero.hp=Math.min(hero.maxHp,hero.hp+hero.maxHp*.15);hero.invuln=1;}
 }
 finishReward(){
  ...
  if(this.wave===1&&!this.bossRoom){this.wave=2;this.mode='play';this.spawnWave();this.emit('wave');}else{this.mode='complete';this.emit('complete');enterShop(this);}
 }
 ...
 nextRoom(){if(this.mode!=='complete')return;if(shopDue(this)){enterShop(this);return;}this.room++;this.wave=1;this.mode='play';this.eliteChallenge=!!this.challengeNext;this.challengeNext=false;if(this.eliteChallenge)this.challengeUsed=true;this.clearBuffers();for(const h of this.heroes){h.hp=Math.min(h.maxHp,h.hp+h.maxHp*.12);h.action=null;h.invuln=1;}this.spawnWave();}
