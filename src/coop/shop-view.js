...
export function handleShopInput(v,slot,input){
 ...
 if(s.inspect){if(input.cancel||input.confirm||input.skill1||input.reroll)sharedOverlay(v,'inspect',null);return;}
 if(s.replacing){if(s.replacing.slot!==slot)return;const r=s.replacing;
  if(input.cancel||input.reroll||input.skill1){sharedOverlay(v,'replacing',null);return;}
...
export function drawShop(v){
 ...
 s.offers.forEach((item,index)=>{
  ...
  text(c,`${GEAR_RARITIES[item.rarity]} · ${ROLES[item.role].name} ${item.slot==='weapon'?'武器':'套装'} · 阶 ${item.level}`,x+20,y+23,14,colors[item.rarity]);
  text(c,v.fit(item.name,260,23,700),x+20,y+53,23,CREAM,'left',700);
  ...
  v.wrap(equipmentText(item),x+125,y+91,256,16,3);
  (item.affixes||[]).forEach((a,i)=>v.wrap(affixes[a.key]||a.key,x+20,y+164+i*27,366,14,1));
  text(c,'点卡面查看数值与换装损益',x+20,y+236,12,'#b8cab0');
  if(!(item.affixes||[]).length)text(c,'基础装备 · 无特殊词条',x+20,y+185,14,'#bfd0b5');
 ...
 text(c,`构筑 ${offer.buildPoints} 点 · 属性 ${offer.attributePoints} 点 · ${core}`,227,523,14,'#e3d4a3');
 v.wrap(h.skills.map((n,i)=>n?`${skillName(h,i)} ${n}`:'未习得技能').join(' / '),227,549,661,14,2);
 v.wrap(Object.entries(h.passives).map(([key,n])=>`${PASSIVES[key]?.title||key} ${n}`).join(' · ')||'暂未持有被动',227,581,661,14,2);
 text(c,v.fit(`武器：${h.equipment?.weapon?.name||'初始'} · 防具：${h.equipment?.armor?.name||'初始'}`,658,14),227,621,14,'#c7d8c0');
 ...
 if(s.replacing){
  ...
  v.button('取消 · B / R / Y',580,480,280,36,()=>sharedOverlay(v,'replacing',null),'#9db795');
 }
 if(s.inspect){
  ...
  if(s.inspect.type==='item'){
   ...
   Object.values(h.equipment).filter(Boolean).forEach((item,i)=>{const y=282+i*148;text(c,`${GEAR_RARITIES[item.rarity]} · ${item.name} / ${equipmentText(item)}`,310,y,17,colors[item.rarity]);item.affixes.forEach((a,j)=>v.wrap(affixText(a),310,y+28+j*38,810,14,2));});
   text(c,'人才稀有度只增加基础生命和攻击强度，初始构筑点不受稀有度影响。',310,591,14,'#c8d8c0');
  }
  v.button('返回商店 · B / A / E',525,645,390,36,()=>sharedOverlay(v,'inspect',null),CYAN);
 }
}
