...
try{
 ...
 const state=await page.evaluate(()=>JSON.parse(render_game_to_text()));assert.deepEqual(state.worldArt,{ready:true,source:'illustrated-png-atlases',pages:6,frames:152});assert.ok(state.characterArt.ready);await shot('menu');
 for(const name of ['forest','enemies','props','icons','ui','effects','projectiles'])assert.ok(requests.some(r=>r.path.endsWith(`/${name}.png`)&&r.status===200));mark('All illustrated atlases and background loaded before gameplay');
 ...
 await page.evaluate(()=>coopTest.start());await page.keyboard.down('a');await page.keyboard.down('ArrowRight');await page.evaluate(()=>advanceTime(440));await page.keyboard.up('a');await page.keyboard.up('ArrowRight');await page.evaluate(()=>advanceTime(650));
