...
const newBodies=expansion?JSON.parse(fs.readFileSync(new URL('source/expansion-20260921/prompts.json',root))).tasks.map(t=>t.key):[];
const BODY_KEYS=expansion?newBodies:['warrior_body_default','warrior_armor_plate','warrior_armor_raider','mage_body_default','mage_armor_star','mage_armor_leaf','archer_body_default','archer_armor_scout','archer_armor_ranger'];
const WEAPON_KEYS=expansion?[]:['warrior_weapon_iron','warrior_weapon_cleaver','mage_weapon_crystal','mage_weapon_ember','archer_weapon_longbow','archer_weapon_crossbow'];
const ROWS=['S','SW','W','NW','N','NE','E','SE'];
const sourceUrl=key=>new URL('source/'+(expansion?'expansion-20260921/':'')+key+'.png',root);
const read=key=>PNG.sync.read(fs.readFileSync(sourceUrl(key)));
...
function handGuess(im,b,key,row){
 ...
 if(!expansion&&role==='warrior'&&!key.includes('default'))return[right?b.x1-b.w*.09:b.x0+b.w*.10,b.y0+b.h*[.60,.57,.52,.55,.55,.56,.55,.59][row]];
 // Skin clusters isolate actual painted mitten, not a generated or patched hand.
 const list=componentsSkin(im,b).filter(p=>p.n>8&&p.n<(expansion?b.w*b.h*.045:900)&&p.cy>b.y0+b.h*(role==='archer'?.65:.49)&&p.cy<b.y0+b.h*.88);
...
// Reviewed atlas-space mitten anchors where fur, trim or a hidden hand defeats skin detection.
const gripOverrides={archer_armor_hawk:[[170,178],[170,174],[102,175],[93,180],[86,177],[165,175],[151,177],[169,176]],archer_armor_fox:{2:[130,175],6:[124,177]},archer_armor_ermine:[[182,173],[180,171],[136,180],[82,170],[79,175],[172,174],[121,179],[163,176]],warrior_armor_crescent:{3:[183,155]},mage_armor_constellation:{2:[132,170]},mage_armor_grove:{2:[137,167],6:[121,166]},mage_armor_rune:{2:[132,163],6:[135,162]}};
for(const key of BODY_KEYS){if(!fs.existsSync(sourceUrl(key)))throw Error('Missing source '+key);const im=read(key),bs=boxes(im,expansion?2:8),height=key.startsWith('mage')?174:160,scale=height/Math.max(...bs.map(b=>b.h)),atlas=new PNG({width:1024,height:expansion?512:2048}),frames=[];
 for(let i=0;i<bs.length;i++){const b=bs[i],row=expansion?i:Math.floor(i/4),column=expansion?0:i%4,cx=i%4*256,cy=Math.floor(i/4)*256,ox=128-headCenter(im,b)*scale,oy=expansion?210-(b.y1+1)*scale:210-height-b.y0*scale;copyScaled(im,b,atlas,cx,cy,scale,ox,oy);const grip=handGuess(im,b,key,row).map((v,k)=>Math.round((v*scale+(k?oy:ox))*10)/10);frames.push({row,column,x:cx,y:cy,w:256,h:256,grip,source:[b.x0,b.y0,b.w,b.h]});}
 if(expansion)for(const frame of frames)if(gripOverrides[key]?.[frame.row])frame.grip=gripOverrides[key][frame.row];
