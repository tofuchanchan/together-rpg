...
function visitShop(w,policy,rec){
 ...
 for(let owner=0;owner<w.humanCount;owner++){
  ...
  if(policy!=='none')hire();
  if(s.actions.length===start&&payer.gold>=shopRerollPrice(w,owner)+24&&w.rerollShop(owner).ok){s.actions.push({type:'reroll',owner});if(policy==='recruit')hire();for(let i=0;i<3&&buy();i++);hire();}
 }
