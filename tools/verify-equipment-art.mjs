...
try{
 ...
 const manifest=await(await fetch('/assets/equipment/manifest.json')).json(),cv=document.createElement('canvas');cv.width=1536;cv.height=1730;cv.style.width='1536px';document.body.replaceChildren(cv);const c=cv.getContext('2d');c.fillStyle='#eff1d9';c.fillRect(0,0,cv.width,cv.height);
 ...
 fs.writeFileSync(out+'/all-directions.png',Buffer.from(data.png.split(',')[1],'base64'));assert.equal(data.checks.length,72);assert.ok(data.checks.every(c=>c.drawn));assert.equal(data.state.bodyFrames,288);assert.deepEqual(errors,[]);
 ...
 fs.writeFileSync(out+'/results.json',JSON.stringify({state:data.state,directions:data.checks,mixed:mixed.cases,protection,errors},null,2));console.log('PASS 72 direction composites + 9 independent slots + 9 rigid-shape/motion/local-hurt protections');
