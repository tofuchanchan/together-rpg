...
try{
 ...
 for(const asset of ['assets/world/manifest.json',coverAsset,'assets/vendor/phaser.min.js']){
  ...
  await screen(p,'error');assert.equal(await p.locator('#retry-loading').isVisible(),true);assert.equal(await p.locator('.loading-monster').isVisible(),false);assert.equal((await state(p)).assetsReady,false);await shot(p,asset===coverAsset?'error-cover':'error-resources');
