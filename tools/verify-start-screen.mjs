...
try{
 ...
 await slow.keyboard.press('Escape');await screen(slow,'title');await slow.locator('#title-start').click();release();await ready(slow);await screen(slow,'setup');assert.equal((await state(slow)).mode,'menu');mark('Slow network: title stays usable, loading shows only an animated monster, then opens setup; Escape cancels waiting');
 ...
 const timeout=await trial.evaluate(async()=>{const {withTimeout}=await import('./src/coop/start-screen.js');try{await withTimeout(new Promise(()=>{}),5,'timed out');}catch(e){return e.message;}});assert.equal(timeout,'timed out');await trial.context().close();mark('Hung-resource timeout helper rejects instead of waiting indefinitely');
