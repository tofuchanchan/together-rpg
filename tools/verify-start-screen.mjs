...
const browser=await chromium.launch({headless:true}),checks=[],errors=[];let coverAsset;
...
try{
 const p=await makePage();await p.goto(base);await screen(p,'title');await ready(p);coverAsset=await p.locator('#title-art').getAttribute('src');await shot(p,'title-desktop');
 ...
 const frame=async(ms,name)=>{await slow.locator('#loading-screen .loading-monster').evaluate((n,ms)=>{const a=n.getAnimations()[0];a.pause();a.currentTime=ms;},ms);await shot(slow,name);return slow.locator('#loading-screen .loading-monster').evaluate(n=>getComputedStyle(n).backgroundPosition);};
 ...
 await slow.keyboard.press('Escape');await screen(slow,'title');await slow.locator('#title-start').click();await slow.evaluate(async()=>{const {startScreen}=await import('./src/coop/start-screen.js?v=0.12.0');startScreen.failGame(new Error('simulated resource timeout'));});await screen(slow,'error');release();await ready(slow);await screen(slow,'setup');assert.equal((await state(slow)).mode,'menu');mark('Slow network: title stays usable, loading shows only an animated monster, then opens setup even after a timeout notice; Escape cancels waiting');
 await slow.context().close();

 for(const asset of ['assets/world/manifest.json',coverAsset,'assets/vendor/phaser.min.js']){
  const p=await makePage();await p.route(`**/${asset}`,route=>route.abort());await p.goto(base,{waitUntil:'domcontentloaded'});
  if(asset!==coverAsset){await screen(p,'title');await p.locator('#title-start').click();}
  await screen(p,'error');assert.equal(await p.locator('#retry-loading').isVisible(),true);assert.equal(await p.locator('#loading-screen .loading-monster').isVisible(),false);assert.equal((await state(p)).assetsReady,false);await shot(p,asset===coverAsset?'error-cover':'error-resources');
  if(asset!==coverAsset){await p.locator('#loading-back').click();await screen(p,'title');await p.locator('#title-start').click();await screen(p,'error');}
 ...
 const timeout=await trial.evaluate(async()=>{const {withTimeout}=await import('./src/coop/start-screen.js?v=0.12.0');try{await withTimeout(new Promise(()=>{}),5,'timed out');}catch(e){return e.message;}});assert.equal(timeout,'timed out');await trial.context().close();mark('Hung-resource timeout helper rejects instead of waiting indefinitely');
 const reduced=await makePage({reducedMotion:'reduce'});await reduced.goto(base);assert.equal(await reduced.locator('#loading-screen .loading-monster').evaluate(n=>getComputedStyle(n).animationName),'none');await reduced.context().close();mark('Reduced-motion preference disables the loading loop');
