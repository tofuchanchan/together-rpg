...
try{
 ...
 await p.goto(new URL('art-lab.html',process.env.GAME_URL||'http://127.0.0.1:4173/').href);await p.waitForFunction(()=>window.artLab?.ready);await p.evaluate(()=>artLab.freeze(.3));await p.locator('#bestiary').screenshot({path:`${out}/bestiary.png`});assert.equal(await p.evaluate(()=>artLab.state.frames),192);mark('All 32 new monster poses render in shared art lab');
