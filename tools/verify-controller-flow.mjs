...
try{
 ...
 await fixture(async()=>{const w=coopTest.world,{rollEquipment}=await import('./src/coop/equipment.js');w.enemies=[];w.pressure=null;w.room=5;w.wave=2;w.mode='complete';w.gold=2000;w.enterShop();w.shop.offers=[rollEquipment(w.heroes[0].role,'armor',5,()=>.9,'controller-armor'),rollEquipment(w.heroes[1].role,'weapon',5,()=>.4,'controller-weapon'),rollEquipment('archer','armor',5,()=>.15,'controller-hunter')];});
 await tap(2);assert.equal((await state()).shop.inspect.type,'item');await tap(1);assert.equal((await state()).shop.inspect,null);await tap(0);s=await state();assert.equal(s.shop.offers[0].sold,true);assert.equal(s.heroes[0].equipment.armor.uid,'controller-armor');const paid=s.gold;await tap(0);assert.equal((await state()).gold,paid);
 await shopCursor(1,2);await tap(0,2);assert.equal((await state()).heroes[1].equipment.weapon.uid,'controller-weapon');
 await hold(0,[3]);s=await state();const rerolls=s.shop.rerolls,gold=s.gold;await step(800);assert.equal((await state()).shop.rerolls,rerolls);assert.equal((await state()).gold,gold);await hold(0,[]);
 mark('Shop detail/B, matching purchases, independent P2 purchase and one-charge held Y reroll work');

 await shopCursor(3);await tap(2);assert.equal((await state()).shop.inspect.type,'recruit');await tap(1);await tap(0);s=await state();assert.equal(s.heroes.length,3);assert.equal(s.heroes[2].ai,true);await tap(3);await shopCursor(3);const beforeRecruit=(await state()).gold;await tap(0);assert.deepEqual((await state()).shop.replacing.choices,[2]);await shot('shop-replace');await tap(1);assert.equal((await state()).shop.replacing,null);assert.equal((await state()).gold,beforeRecruit);await tap(0);await tap(0);s=await state();assert.equal(s.heroes[2].name,s.shop.recruit.name);assert.equal(s.heroes[0].ai,false);assert.equal(s.heroes[1].ai,false);await shopCursor(5);await tap(0);assert.equal((await state()).mode,'shop');await shopCursor(5,2);await tap(0,2);assert.equal((await state()).mode,'complete');
