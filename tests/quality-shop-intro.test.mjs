...
function store(key='stars',humans=2){const w=new World(842),p=SKILL_PAIRS[key];w.reset([p.role,p.role],humans);for(const h of w.heroes)ready(h,key);w.clears=19;w.room=10;w.mode='complete';w.heroes.forEach(h=>h.gold=500);w.enterShop();return w;}
function stock(w,key,slot=0){const h=w.heroes[slot],o=skillPool(h,{clears:w.clears}).find(o=>o.key===key);assert.ok(o);w.shop.stalls[slot].lessons[0]={...o,uid:`test-lesson-${slot}`,role:h.role,targetRank:o.kind==='passive'?(h.passives[key.split(':')[1]]||0)+1:o.kind==='active'?h.skills[o.slot]+1:null,price:lessonPrice(o,w.room)};return w.shop.stalls[slot].lessons[0];}
...
test('mastery purchase checks both recipe and owner, spends once and cannot be purchased again after reroll',()=>{
 const w=store(),item=stock(w,'mastery:stars'),gold=w.heroes[0].gold;w.heroes[1].passives.arcane=1;assert.ok(lessonEligibility(w,item,w.heroes[1]));assert.equal(w.buyLesson(1,1,item.uid).ok,false);assert.equal(w.heroes[0].gold,gold);assert.equal(w.buyLesson(0,0,item.uid).ok,true);assert.equal(w.heroes[0].pairMastery.stars,true);assert.equal(w.heroes[0].gold,gold-item.price);assert.equal(w.buyLesson(1,0,item.uid).ok,false);w.rerollShop(0);assert.ok(!w.shop.stalls[0].lessons.some(o=>o?.uid===item.uid));assert.equal(w.shop.stalls[0].lessonPurchases,1);assert.equal(w.buyLesson(0,0,item.uid).ok,false);
});
test('unaffordable and stale lesson purchases preserve all progression, money and RNG',()=>{
 const w=store(),item=stock(w,'mastery:stars');w.heroes[0].gold=1;const seed=w.seed,hero=structuredClone(w.heroes[0]);assert.equal(w.buyLesson(0,0,item.uid).ok,false);assert.equal(w.heroes[0].gold,1);assert.equal(w.seed,seed);assert.deepEqual(w.heroes[0],hero);w.heroes[0].gold=300;w.rerollShop(0);const gold=w.heroes[0].gold;assert.equal(w.buyLesson(0,0,item.uid).ok,false);assert.equal(w.heroes[0].gold,gold);
});
...
test('lesson details and purchase surcharges are independent for both controller slots',()=>{
 const w=store();stock(w,'mastery:stars');stock(w,'mastery:stars',1);const v={world:w,router:{flush(){assert.fail('personal actions must not flush partner input');}}};w.shop.stalls.forEach(s=>{s.category='skills';s.cursor=0;});
 handleShopInput(v,0,{skill1:true});assert.deepEqual(w.shop.stalls[0].inspect,{type:'lesson',index:0});handleShopInput(v,1,{confirm:true});assert.equal(w.heroes[1].pairMastery.stars,true);assert.equal(w.heroes[0].pairMastery.stars,undefined);
 handleShopInput(v,0,{cancel:true});assert.equal(w.heroes[0].gold,500);handleShopInput(v,0,{confirm:true});assert.equal(w.heroes[0].pairMastery.stars,true);assert.equal(w.shop.stalls[0].lessons[0].sold,true);assert.equal(w.shop.stalls[1].lessons[0].sold,true);
});

test('lessons can target a recruited companion and a new shop resets the purchase surcharge',()=>{
 const w=store('stars',1),h=createHero('mage',1,{ai:true});ready(h,'stars');w.heroes.push(h);const item=stock(w,'mastery:stars');assert.equal(w.buyLesson(0,1,item.uid).ok,true);assert.equal(h.pairMastery.stars,true);w.leaveShop(0);w.room=15;w.mode='complete';w.enterShop();assert.equal(w.shop.stalls[0].lessonPurchases,0);assert.ok(!w.shop.stalls[0].lessons[0].sold);
});
