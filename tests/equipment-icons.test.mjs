...
test('draw resolves by appearance ID, independent of rarity or catalog ordering; bonus art still uses its atlas',async()=>{
 ...
 try{
  ...
  for(const [key,icon] of Object.entries(manifest.icons).reverse())for(const rarity of ['common','legendary']){assert.equal(drawEquipmentIcon(ctx,{visualKey:key,rarity},10,20,64),true);const call=draws.at(-1);assert.ok(call[0].url.endsWith(icon.file));assert.deepEqual(call.slice(1,5),icon.frame);assert.equal(Math.round(Math.max(call[7],call[8])),64);}
  ...
  drawBonusCreature(ctx,{bonusKind:'gold',stride:.6,stats:{size:350}});assert.ok(draws.at(-1)[0].url.endsWith('bonus.png'));assert.deepEqual(draws.at(-1).slice(1,5),manifest.bonus.frames[6]);
